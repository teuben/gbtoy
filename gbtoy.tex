...
\centering
  ...
  \caption{\label{fig1} SDFITS BINTABLE: spectra data and meta-data.
    Typically different rows in the DATA are manipulated (ON/OFF and averaging)
    to result in a final single spectrum for a given RA/DEC.}
\end{figure}
...
\caption{\label{fig2} Evolution of the GBT analysis software?}
\end{figure}
...
\caption{\label{fig2} Example GBTIDL spectrum after baseline, not matching the example code!}
\end{figure}
...
In figure\ref{fig3} we show the result of running {\tt sdimage}, which creates a 2D image ``Waterfall Plot''
...
\centering
  ...
  \caption{\label{fig3} Viewing an sdimage version of ngc5291 with CARTA:
    full view and zoomed version on the HI spectral line. This kind
    of image is also known as as {\bf Waterfall Plot}. Channels run along X, time/sequence along Y.}
\end{figure}
